...
def table_detection(img_path, show_img = False):
    ...
    while cells:
        ...
        # print(row_cells)
        row_cells = reduce_col(row_cells,20)
    ...
    for row in rows:
        ...
        for cell in row:
            ...
            if char_sized_bounding_rects:
                ...
                # if min(iw + FIXED_PX*2, w) < MIN_WIDTH or min(ih + FIXED_PX*2, h) < MIN_HIGHT:
                #     continue
    ...
    descartes = []
    # read file 
    annot_data = list(parseXml(xml_path))
    ...
    for row in rows:
        print(row)
        for box in row:
            cv2.rectangle(t_row, (box[0], box[1]),
                                          (box[0]+box[2], box[1]+box[3]), 255, 2)
    print("=================")
    ...
    for row in new_rows:
        print(row)
        for box in row:
            cv2.rectangle(t_n_row, (box[0], box[1]),
                          (box[0]+box[2], box[1]+box[3]), 255, 2)
    # showImage(t_row)
    # showImage(t_n_row)

    for true_row in annot_data:
        ious = cal_iou_row(true_row, new_rows[idx], test2)
        descartes += ious 
        idx += 1
    ...
    # average = sum(descartes) / cell_count
    print(descartes)
    return sum(descartes > 0.5) / descartes.shape[0]
    print(good_estimated/descartes.shape[0])
    # print(good_estimated / len(descartes))
    # return good_estimated/ len(descartes)
